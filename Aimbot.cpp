...
bool Aimbot::HitChance(C_BaseEntity* pEnt, C_BaseCombatWeapon* pWeapon, Vector Angle, Vector Point, int chance)
{
	...
	if (Backtrack[pEnt->EntIndex()] || ShotBacktrack[pEnt->EntIndex()]) // doing this bec im lazy
	{
		...
		float SpreadCone = pWeapon->GetAccuracyPenalty() * 256.0f / M_PI + pWeapon->GetCSWpnData()->max_speed * Velocity / 3000.0f;
	...
	else if (Backtrack[pEnt->EntIndex()] || Backtrack[pEnt->EntIndex()])
	{
		float Velocity = g::pLocalEntity->GetVelocity().Length();
...
bool SlowWalkBaim(C_BaseEntity* entity)
{	
	static float oldSimtime[65];
	static float storedSimtime[65];
	static bool BaimShot[65];
	...
	if (!g_Menu.Config.SlowWalkBaim)
		return false;
	
	if (entity->GetVelocity().Length2D() > 25.f && entity->GetVelocity().Length2D() < 40.f)
		Baim[entity->EntIndex()] = true;
	else
		return false;
}
