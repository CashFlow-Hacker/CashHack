...
void Hooks::Init()
{
	...
	g_pSurface->SetFontGlyphSet(g::WeaponFont, "TINYBBA_", 8, 16, 0, 0, FONTFLAG_OUTLINE); //300 
	g::CourierNew = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::CourierNew, "TINYBBA_", 14, 28, 0, 0, FONTFLAG_DROPSHADOW); //300 
	g::Verdana = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::Verdana, "TINYBBA_", 12, FW_BOLD, 0, 0, FONTFLAG_OUTLINE); //700 
	g::NiggaPee = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::NiggaPee, "TINYBBA_", 9, 500, 0, 0, FONTFLAG_OUTLINE);
	g::SauIsAFatRetard = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::SauIsAFatRetard, "TINYBBA_", 9, 500, 0, 0, FONTFLAG_OUTLINE);
	g::IndicatorFont = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::IndicatorFont, "TINYBBA_", 27, FW_EXTRABOLD, 0, 0, FONTFLAG_DROPSHADOW);
	g::FlagFont = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::FlagFont, "TINYBBA_", 10, FW_EXTRABOLD, 0, 0, FONTFLAG_OUTLINE);
	g::memefont = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::memefont, "TINYBBA_", 12, FW_THIN, 0, 0, FONTFLAG_OUTLINE);
	g::supfont = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::supfont, "TINYBBA_", 15, 15, 0, 0, FONTFLAG_NONE);
	g::smallestpixel = g_pSurface->FontCreate();
	g_pSurface->SetFontGlyphSet(g::smallestpixel, "TINYBBA_", 14, 500, 0, 0, FONTFLAG_OUTLINE);

	g_pEngine->ExecuteClientCmd("clear");
	
	Warning("\n               Welcome to Cash Hack\n`"); 
...
void DrawBeam(Vector src, Vector end, Color color, float life)
{
	...
	beamInfo.m_vecStart = src + Vector(0,0,5);
...
static bool buybotfix; // again what have you done i was afk?
...
void Event::FireGameEvent(IGameEvent* event)
{
	...
	if (buybotfix)
	{
		...
		{
			//ammo
			if (g_Menu.Config.BuyBot.ammo == 0)
	...
	if (entityid == g_pEngine->GetLocalPlayer() && userentityid != g_pEngine->GetLocalPlayer())
	{
		...
		if (nUserID || nDead)
		{
			...
			std::string hit = (" \x0D[Cash] \x01hit"" ");
