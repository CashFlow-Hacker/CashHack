...
void ESP::latency() {
	...
	if (g::pLocalEntity->IsAlive()) {
		int width, screeny;
		g_pEngine->GetScreenSize(width, screeny);

		auto net_channel = g_pEngine->GetNetChannelInfo();
		int ping = (int)(net_channel->GetLatency(FLOW_OUTGOING) * 1000);
		std::stringstream ss;
		ss << ping << " MS";
		if (g_Menu.Config.IndicatorLatency)
			g_pSurface->DrawT(10, screeny - 110, Color(235, 140, 52), indicatorfont, false, ss.str().c_str());
	}
}
...
void ESP::Arrows()
{
	...
	//	if (g_Aimbot.baiming)
	//		g_pSurface->DrawT(10, screeny - 70, Color(255, 0, 0, 255), indicatorfont, false, ("BAIM"));
		if (g_Menu.Config.AAInvertIndicators)
		g_AntiAim.invert ? g_pSurface->DrawT(10, screeny - 70, Color(0, 255, 0, 255), indicatorfont, false, ("LEFT")) : g_pSurface->DrawT(10, screeny - 70, Color(0, 255, 0, 255), indicatorfont, false, ("RIGHT"));
		...
		if (g_Menu.Config.Antiaim && g::pLocalEntity->IsAlive() && g_Menu.Config.DesyncType != 6)
		{
			...
			g_pSurface->DrawT(10, screeny - 100, Color(rcolor, gcolor, 0, 255), indicatorfont, false, ("DSY"));
			if (g_Menu.Config.LBYDelta > 0)
				g_pSurface->DrawT(10, screeny - 130, abs(g::RealAngle.y - g::pLocalEntity->GetLowerBodyYaw()) > (g_Menu.Config.Jitter + g_Menu.Config.LBYDelta - 10) ? Color(0, 255, 0, 255) : Color(255, 0, 0, 255), indicatorfont, false, ("LBY"));
		}
		if (g_Menu.Config.Antiaim && g::pLocalEntity->IsAlive() && g_Menu.Config.DesyncType == 6)
		{	
			if (g_Menu.Config.LBYDelta > 0)
				g_pSurface->DrawT(10, screeny - 100, abs(g::RealAngle.y - g::pLocalEntity->GetLowerBodyYaw()) > (g_Menu.Config.Jitter + g_Menu.Config.LBYDelta - 10) ? Color(0, 255, 0, 255) : Color(255, 0, 0, 255), indicatorfont, false, ("LBY"));
		}
		...
		{			
			...
			if (g_Menu.Config.LBYDeltaLegit > 0)
				g_pSurface->DrawT(10, screeny - 130, abs(g::RealAngle.y - g::pLocalEntity->GetLowerBodyYaw()) > (g_Menu.Config.Jitter + g_Menu.Config.LBYDeltaLegit - 10) ? Color(0, 255, 0, 255) : Color(255, 0, 0, 255), indicatorfont, false, ("LBY"));
		} 
		...
		if (g::pLocalEntity->zeus() && g_Menu.Config.Aimbot) {
			...
			g_pSurface->DrawT(screen2.x, screen2.y, Color(255, 255, 255, 125), niggafont, false, ("RANGE"));
		}
		...
		if (g_Menu.Config.Arrows) {
			if (g_Misc.isthirdperson) {
				...
				g_pSurface->DrawT(screen2.x, screen2.y, Color(128, 244, 66, 255), niggafont, false, ("REAL"));
				...
				g_pSurface->DrawT(screen2.x, screen2.y, Color(66, 124, 244, 255), niggafont, false, ("LBY"));
				...
				g_pSurface->DrawT(screen2.x, screen2.y, Color(244, 100, 66, 255), niggafont, false, ("DSY"));
				...
				g_pSurface->DrawT(screen2.x, screen2.y, Color(75, 192, 235, 255), niggafont, false, ("GOAL"));
...
void ESP::projectiles(C_BaseEntity* entity) {
	...
	if (model) {
		...
		if (name.find("flashbang") != std::string::npos) {
			g_pSurface->DrawT(grenade_position.x, grenade_position.y, Color(255, 255, 255, 255), niggafont, true, ("FLASHBANG"));
		}

		else if (name.find("smokegrenade") != std::string::npos) {
			g_pSurface->DrawT(grenade_position.x, grenade_position.y, Color(155, 155, 155, 255), niggafont, true, ("SMOKE"));
		}

		else if (name.find("incendiarygrenade") != std::string::npos) {
			g_pSurface->DrawT(grenade_position.x, grenade_position.y, Color(244, 100, 66, 255), niggafont, true, ("INCENDIARY"));
		}

		else if (name.find("molotov") != std::string::npos) {
			g_pSurface->DrawT(grenade_position.x, grenade_position.y, Color(244, 100, 66, 255), niggafont, true, ("MOLOTOV"));
		}

		else if (name.find("fraggrenade") != std::string::npos) {
			g_pSurface->DrawT(grenade_position.x, grenade_position.y, Color(244, 100, 66, 255), niggafont, true, ("HEGRENADE"));
		}

		else if (name.find("decoy") != std::string::npos) {
			g_pSurface->DrawT(grenade_position.x, grenade_position.y, Color(128, 244, 66, 255), niggafont, true, ("DECOY"));
		}
	}
}
...
void ESP::newbombtimer(C_BaseEntity* ent) // probably the most retarded thing you've ever seen
{
	...
	if (g_Menu.Config.BombEsp)
	{
		...
		if (Defuser)
		{
			...
			g_pSurface->DrawT(bomb_position.x, bomb_position.y + 12, Color(255, 0, 0), niggafont, true, ("DEFUSING"));
		...
		if (!Utils::WorldToScreen(bomb_origin, bomb_position))	return;
		g_pSurface->DrawT(bomb_position.x, bomb_position.y, Color(255, 255, 255), niggafont, true, ("BOMB"));
	}
}
...
void ESP::watermark() // who needs gettextsize LOL
{
	...
	int ping = (int)(net_channel->GetLatency(FLOW_OUTGOING) * 1000);
	int engineClientVersion = g_pEngine->GetClientVersion();
	ss << (" CashHack | ") << data << (" | ping: ") << ping << (" | client version: ") << engineClientVersion;
	...
	int zy = 450;
	z = 50;
	g_pSurface->FilledRect(width - x - z, 4, y + z - 1, 20, Color(48, 49, 51, 235));
	g_pSurface->DrawT(width - zy, 6.5f, Color(194, 200, 209, 245), supremacy, false, ss.str().c_str());
}
